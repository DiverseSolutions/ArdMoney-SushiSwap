...
const { initializeDummyTokens, initializeArdMoneyContracts, approveToken, tokenMint } = require("./helpers.js");
...
describe("ArdMoney Swap Dynamic Pool", function () {
  ...
  this.beforeEach(async function () {
    ...
    [factory, router, weth] = await initializeArdMoneyContracts(
      feeSetter.address,
      routerAdmin.address,
      swapFee,
      mintFee
  ...
  it("Swap Dynamic Pool", async function () {
    ...
    await router.connect(amaraa).swapExactTokensForTokens(amountInWei, 1, path, amaraa.address, fakeDeadline);
