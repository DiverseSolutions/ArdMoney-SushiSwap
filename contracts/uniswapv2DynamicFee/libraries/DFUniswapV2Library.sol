...
import "../interfaces/IDFUniswapV2Pair.sol";
...
library UniswapV2Library {
    ...
    function getAmountsOut(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 swapFee =  IUniswapV2Pair(pairFor(factory, path[i], path[i + 1])).getSwapFee();
    ...
    function getAmountsIn(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            uint256 swapFee =  IUniswapV2Pair(pairFor(factory, path[i - 1], path[i])).getSwapFee();
